...
function generateAirWaybillHTML(booking) {
    const {
        awb,
        consigneeName,
        consigneeEmail,
  ...
  const todayDate = new Date().toLocaleString('en-GB', {
    ...
    // Format rate
    const formattedRate = rate ? new Intl.NumberFormat().format(rate) : '';
  
    // Calculate total weight and formatted final weight
    const formattedFinalWeight = total ? Math.floor(total) : 0;
  
    // Extract flight code
    const flightCode = flightName ? flightName.substring(0, 2) : '';
  
    // Find the largest package by dimension
    const largestPackage = packages.length > 0
      ? packages.reduce((largest, current) => {
          const currentLength = current.packageLength || 0;
          const currentWidth = current.packageWidth || 0;
          const currentHeight = current.packageHeight || 0;
          const currentDimension = Math.max(currentLength, currentWidth, currentHeight);
  
          const largestLength = largest.packageLength || 0;
          const largestWidth = largest.packageWidth || 0;
          const largestHeight = largest.packageHeight || 0;
          const largestDimension = Math.max(largestLength, largestWidth, largestHeight);
  
          return currentDimension > largestDimension ? current : largest;
        }, packages[0])
      : {};
  
    // Format largest package dimension
    const dimension = largestPackage.packageLength && largestPackage.packageWidth && largestPackage.packageHeight
      ? `${largestPackage.packageLength}x${largestPackage.packageWidth}x${largestPackage.packageHeight}`
      : '';
  ...
  let to='';
  let from='';
  if(fromRegion==='Bossaso'){
    to='BSA';
    from='BSA';
  }
  else if(fromRegion==='Dubai'){
    to='DXB';
    from='DXB';
  }
  else if(fromRegion==='Garowe'){
    to='GGR';
    from='GGR';
  }
  else if(fromRegion==='Hargeisa'){
    to='HGA';
    from='HGA';
  }
  else if(fromRegion==='Jeddah'){
    to='JED';
    from='JED';
  }
  else if(fromRegion==='Djibouti'){
    to='JIB';
    from='JIB';
  }
  else if(fromRegion==='Madinah'){
    to='MED';
    from='MED';
  }
  else if(fromRegion==='Mogadishu'){
    to='MGQ';
    from='MGQ';
  }
  else if(fromRegion==='Nairobi'){
    to='NBO';
    from='SHJ';
  }
  else if(fromRegion==='Sharjah'){
    to='SHJ';
    from='SHJ';
  }
  let start='';
  ...
  *{
  ...
  <div id="main">
      <div id="row1">
             <div id="box1">
              Shipper's Name,Address<br><br>
              ${shipperName}<br>
              ${shipperAddress}<br>
              ${shipperCity},${shipperCountry}<br>
              Tel: ${shipperPhone} <br>
          ...
          <div id="r1b2">
              ...
              Air Waybill<br>
              Issued By <br>
              <div class="logo"><img src="${imagePath}" alt="Daallo Airlines Logo"></div>
          </div>
      </div>
      <div id="row2">
          <div id="row2-1">
              <div id="box2">
                  ...
                  ${consigneeName}<br>
                  ${consigneeAddress}<br>
                  ${consigneeCity},${consigneeCountry}<br>
          ...
          It is agreed that goods described here are apparent good order and condition (except as noted) for carriege SUBJECT TO THE CONDITIONS OF CONTRACT ON THE REVERSE HEREOF. THE SHIPPER'S ATTENTION IS DRAWN TO THE NOTICE CONCERNING CARRIERS LIMITATION OF LIABILITY. Shipper may increase such indication of liability by declaring ahigher value for carriege and paying a supplemental charge if required. ALL GOODS MAY BE CARRIED BY ANY OTHER MEANS INCLUDING ROAD OR ANY OTHER CARRIER UNLESS SPECIFIC CONTRARY INSTRUCTIONS ARE GIVEN HEREON BY THE SHIPPER, AND SHIPPER AGREES THAT THE SHIPMENT MAY BE CARRIED VIA INTERMEDIATE STOPPING PLACES WHICH THE CARRIER DEEMS APPROPRIATE.
           ...
           ${paymentTerms} <br>
           SECURITY ID:0 | PRIORITY:${goodsDescriptionsCodes}
           <br>
          </div>
      ...
      <div id="row3-2">
           ...
           <div id="r3-2-4">
              Declared value for <br> Carriage <br>    NDV
           </div>
           <div id="r3-2-5">
              Declared value for <br> Customers <br> NVC
           </div>
      </div>
  </div>
  <div id="row4">
      ...
      <div id="row7">
          <div id="box10-1"><p>${packageCount}</p>
              ...
              <div id="box10-2"><p>${finalWeight}</p>
                  <p id="b10-2-1">${finalWeight}</p>
              </div>
              ...
              <div id="box10-7"><p>${rate}</p></div>
              <div id="box10-8"><p>${goodsDescriptionsCodes} CARGO ${packageCount} @ ${dimension} CM</p>
              </div>
      </div>
      <div id="row8">
          <div id="box11-1">
          ...
          <div id="box11-2">
              <div id="line">
              <p style="margin-left:100px;" font = "8px">${rate}</p>
              </div>
          </div>
          <div id="box11-3">
              <div id="valuation">Valuation Charge</div>
              <div id="line2"></div>
          </div>
      </div>
      <div id="row9">
      ...
      <div id="row10">
          ...
          <div id="row11">
              ...
              <div id="row11-2">
                  <br><br><br><br><br> ${todayDate} ${createdBy}
                  <hr class="dotted-hr">
...
module.exports = { generateAirWaybillHTML };
